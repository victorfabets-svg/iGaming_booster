...
async function ensureMigrationsTable() {
    ...
    await connection_1.db.query(`CREATE SCHEMA IF NOT EXISTS events;`);
...
async function getExecutedMigrations() {
    const result = await connection_1.db.query(`SELECT filename, executed_at FROM events.migrations ORDER BY executed_at ASC`);
    return result.rows;
}
async function markMigrationExecuted(filename) {
    await connection_1.db.query(`INSERT INTO events.migrations (filename, executed_at) VALUES ($1, NOW())`, [filename]);
}
