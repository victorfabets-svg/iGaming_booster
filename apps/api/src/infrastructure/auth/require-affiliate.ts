/**
 * requireAffiliateOrAdmin middleware - enforces affiliate or admin role for protected routes.
...
/**
 * requireAffiliateOrAdmin middleware factory
 * @param fastify - Fastify instance for DB access if needed
 */
export function requireAffiliateOrAdmin(fastify: FastifyInstance) {
  return async function requireAffiliateOrAdminHandler(
    ...
    try {
      ...
      if (userRole !== 'admin' && userRole !== 'affiliate') {
        return reply.status(403).send({ 
          success: false, 
          error: { code: 'FORBIDDEN', message: 'Affiliate or admin role required' } 
        });
      }
